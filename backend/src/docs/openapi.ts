...
export const openApiDocument = {
  ...
  info: {
    ...
    description: [
      ...
      "```",
      "",
      "Frontend/client socket can connect once, then subscribe or unsubscribe without reconnecting:",
      "```json",
      ...
      "```",
      "Subscriber receives `device_info` when a device socket connects/disconnects. Only one `is_device=1` socket is allowed per sensor.",
      "```json",
      '{ "type": "device_info", "success": true, "data": [{ "sensor_code": "00001", "connection_status": false }] }',
      "```",
      "",
  ...
  paths: {
    ...
    "/api/websocket-log": {
      get: {
        tags: ["WebSocket Log"],
        summary: "List websocket logs",
        security: [{ cookieAuth: [] }],
        parameters: [
          ...paginatedQuery,
          { name: "level", in: "query", schema: { type: "string", enum: ["info", "warning", "error"] } },
          { name: "date", in: "query", schema: { type: "string", example: "2026-09-25" } },
          { name: "start_time", in: "query", schema: { type: "string", example: "08:00" } },
          { name: "end_time", in: "query", schema: { type: "string", example: "17:00" } },
        ],
        responses: { 200: ok() },
      },
      delete: {
        tags: ["WebSocket Log"],
        summary: "Clear websocket logs",
        security: [{ cookieAuth: [] }],
        parameters: [
          ...paginatedQuery,
          { name: "level", in: "query", schema: { type: "string", enum: ["info", "warning", "error"] } },
