...
import { Button, FilterGrid, FormInput, Modal, Table } from "../../../components/ui";
import {
  useClearWebSocketLogMutation,
  useWebSocketLogListQuery,
} from "../../../api/websocketLog/websocketLogQuery";
...
export const WebSocketLogPage = () => {
  usePageTitle("WebSocket Log Monitoring");
  ...
  const { handleApiFormError } = useApiFormError({ logEvent: "websocket_log_clear_failed" });
  const { mutate: clearLogMutation, isPending: isClearLogPending } =
    useClearWebSocketLogMutation();
    ...
    clearLogMutation(
      ...
      },
      {
        onSuccess: (response) => {
          ...
          void queryClient.invalidateQueries({ queryKey: ["websocket-log"] });
  ...
  return (
    <>
      <PageHeader
        title="WebSocket Log Monitoring"
        subtitle="Review connection, handshake, subscribe, and message errors from the WebSocket server."
        breadcrumbs={[
          { label: "System", route: undefined },
          { label: "WebSocket Logs", route: undefined },
        ]}
        rightElement={
          hasAccess("D") ? (
            <Button
      ...
      <FilterGrid>
        <FormInput
          ...
          type="text"
          icon={Search}
          placeholder="Search websocket logs by message, context, or IP address..."
        />
      ...
      <Table
        ...
        emptyText="No websocket logs available yet."
        showNumber={false}
      ...
      <Modal
        ...
        title="Clear websocket logs"
        subtitle="This action removes websocket log records that match the current filter."
        className="max-w-lg"
        footer={
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-dark-900">
                You are about to clear the filtered websocket log entries.
              </p>
